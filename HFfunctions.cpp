...
{
  if(m==n){ std::cerr << "number_level(m,n,M) m=" << m << " should not equal n=" << n << std::endl; exit(1); }
...
void Shells::resize(size_t size)
{
  sp_ind.resize(size);
  pn_ind.resize(size);
  tz.resize(size);
...
void States::resize(size_t size)
{
  sp_ind.resize(size);
  pn_ind.resize(size);
  tz.resize(size);
...
J_Matrix_Elements::J_Matrix_Elements(int pl, int nl, std::vector<double> pjs, std::vector<double> njs)
{
  double maxpj = -1.0;
  double maxnj = -1.0;
...
void J_Matrix_Elements::set_ppJME(int m, int n, int l, int k, double J, double ME)
{
  ...
  double factor = 1.0;
  if(m > n){ factor *= pow(-1.0, int(pjvec[m] + pjvec[n] + J + 1.0)); std::swap(m, n); }
  if(l > k){ factor *= pow(-1.0, int(pjvec[l] + pjvec[k] + J + 1.0)); std::swap(l, k); }
  branum = number_shell(m, n, pshelltot);
  ketnum = number_shell(l, k, pshelltot);
  if(branum > ketnum){ std::swap(branum, ketnum); }
...
void J_Matrix_Elements::set_nnJME(int m, int n, int l, int k, double J, double ME)
{
  int branum, ketnum, vnum;
  double factor = 1.0;
  if(m > n){ factor *= pow(-1.0, int(njvec[m] + njvec[n] + J + 1.0)); std::swap(m, n); }
  if(l > k){ factor *= pow(-1.0, int(njvec[l] + njvec[k] + J + 1.0)); std::swap(l, k); }
...
{ 
  ...
  std::string path; // path for input file
  std::string line; // string for each file line
  std::ifstream filestream;	// input file
  int index; // count to keep track of input line
  size_t colon; // string element equal to ':'
  std::string substr; // substring of relevant information
  ...
  std::cout << "Level Scheme = " << Input.LevelScheme << std::endl;
  std::cout << "Matrix Elements = " << Input.MatrixElements << " " << Input.COMMatrixElements << std::endl << std::endl;
...
{
  std::cout << "Reading Model Space File" << std::endl;
  std::cout << "------------------------" << std::endl;

  Model_Space Space; // Model Space information
  std::string fullpath; // Model Space file path
  std::string phline, number; // phline - string for each file line, number - string for first word of each line
  ...
  std::vector<int> indvec, nvec, lvec, j2vec, tvec, l2nvec; // initialize corresponding vectors
  double energy; // level energy
  std::stringstream a, b; // streams for naming shells
  std::string a1, b1, c1; // strings for naming shells
  std::string shell, tz; // strings for naming shells
  ...
  std::cout << "A = " << Space.A << ", HOEnergy = " << Space.HOEnergy << ", TotOrbs = " << TotOrbs << std::endl;
  getline(splevels, phline);
  phstream.str(phline);
  phstream >> number;
  ...
  if(Space.Pocc == 0 || Space.Nocc == 0){ std::cerr << "Not Closed-Shell" << std::endl; exit(1); }
...
Single_Particle_States Build_Single_Particle_States(const Input_Parameters &Parameters, const Model_Space &Space)
{
  ...
  //Get Energies
  for(int i = 0; i < Space.pshelltot; ++i){ States.p_energies[i] = Space.pShells.energy[i]; }
  for(int i = 0; i < Space.nshelltot; ++i){ States.n_energies[i] = Space.nShells.energy[i]; }
  ...
  for(int i = 0; i < Space.pshelltot - 1; ++i){
    ...
    std::swap(States.p_energies[i], States.p_energies[ind]);
    std::swap(States.protons[i], States.protons[ind]);
    std::swap(States.p_j[i], States.p_j[ind]);
    std::swap(States.p_l[i], States.p_l[ind]);
  }
  ...
  for(int i = 0; i < Space.nshelltot - 1; ++i){
    ...
    std::swap(States.n_energies[i], States.n_energies[ind]);
    std::swap(States.neutrons[i], States.neutrons[ind]);
    std::swap(States.n_j[i], States.n_j[ind]);
    std::swap(States.n_l[i], States.n_l[ind]);
  }
...
void Read_J_Matrix_Elements(const std::string &MEfile, const Model_Space &Space, J_Matrix_Elements &ME)
{
  ...
  for(int i = 0; i < NumElements; ++i){
    ...
    if(Space.spShells.tz[shell1-1] == 1 && Space.spShells.tz[shell2-1] == -1)
      { std::swap(shell1, shell2); TBME *= pow(-1.0, Space.spShells.j[shell1-1] + Space.spShells.j[shell2-1] - coupJ); }
    if(Space.spShells.tz[shell3-1] == 1 && Space.spShells.tz[shell3-1] == -1)
      { std::swap(shell3, shell4); TBME *= pow(-1.0, Space.spShells.j[shell3-1] + Space.spShells.j[shell4-1] - coupJ); }
  ...
  std::cout << std::endl << std::endl << "!!!!" << int1 << "!!!!" << std::endl << std::endl;
...
{
  std::cout << "Reading J-Matrix Elements" << std::endl;
  std::cout << "-----------------------" << std::endl;
...
Single_Particle_States Hartree_Fock_States2(const Input_Parameters &Parameters, const Single_Particle_States &States, const Model_Space &Space, const J_Matrix_Elements &ME)
{
  ...
  std::vector<double> pdensmat, ndensmat, pfock, nfock; // Fock Matrix
  std::vector<int> brakvec; // Braket for Matrix Element Permutation
  char jobz, uplo; // Parameters for Diagonalization, Multiplication
  int plda, nlda; // Parameter for Diagonalization
  std::vector<double> pw, pwork, nw, nwork; // EigenEnergy Vector and work vector
  ...
  while((error > 1e-8 && ind < 100) || ind < 10){
    ...
    for(int i = 0; i < int(HF.holes.size()); ++i){ std::cout << HF.h_energies[i] << " "; }
    std::cout << std::endl;
    /*for(int i = 0; i < int(HF.holes.size()); ++i){ std::cout << HF.h_j[i] << " "; }
    std::cout << std::endl;
    for(int i = 0; i < int(HF.holes.size()); ++i){ std::cout << HF.h_l[i] << " "; }
    std::cout << std::endl;*/
	...
	for(int beta = 0; beta < Space.Pocc; ++beta){ // Sum over occupied shells
	  ...
	  for(int J = minj; J <= maxj; ++J){
		...
		if(ind == 1 && m == 0 && l == 5){
		  ++tempint;
		  //std::cout << Space.pShells.sp_ind[m]+1 <<" "<< Space.pShells.sp_ind[n]+1 <<" "<< Space.pShells.sp_ind[l]+1 <<" "<< Space.pShells.sp_ind[k]+1 <<"   "<< J <<"   "<< HF.protons[beta][n] <<" "<< HF.protons[beta][k] <<" "<< (2*J + 1)/(2*Space.pShells.j[m]+1) <<" "<< ME.get_ppJME(m,n,l,k,J) <<"   "<< term << std::endl;
		}
	      }
	    }
	  }
    ...
    /*for(int i = 0; i < P; ++i){
      for(int j = 0; j < P; ++j){
	if(i == j){ std::cout << i+1 << " " << Space.pShells.energy[i] << " " << pfock[P * i + j] - Space.pShells.energy[i] << std::endl; }
      }
      }*/
    ...
    for(int i = 0; i < int(pfock.size()); ++i){ if(fabs(pfock[i]) < 10e-13){ pfock[i] = 0; }}
    for(int i = 0; i < int(nfock.size()); ++i){ if(fabs(nfock[i]) < 10e-13){ nfock[i] = 0; }}
    ...
    std::cout << std::endl;
    for(int i = 0; i < N; ++i){ std::cout << nw[i] << " "; }
    std::cout << std::endl;
    
    for(int i = 0; i < P; ++i){
      error += fabs(HF.p_energies[i] - pw[i]); 
      HF.p_energies[i] = pw[i];
    }
    for(int i = 0; i < N; ++i){
      error += fabs(HF.n_energies[i] - nw[i]); 
  ...
  std::vector<int> p_n(HF.protons.size());
  std::vector<int> n_n(HF.neutrons.size());
...
{
  std::cout << "Converting Matrix Elements from J-Scheme to HF J-Scheme" << std::endl;
  std::cout << "----------------------------------------------------" << std::endl;
  ...
  int num1, num2;
  double tempel;
  std::vector<double> PP_M1, PP_M2, NN_M1, NN_M2, PN_M1, PN_M2; // Matrices of coefficients
  std::vector<double> PP_V, NN_V, PN_V;
  std::vector<double> PP_C, NN_C, PN_C;
  char transa, transb;
  double alpha1, beta1;
  int ind;
  ...
  for(int J = 0; J <= Space.max2J; ++J){
	...
	for(int beta = 0; beta < plength; ++beta){
	  for(int delta = 0; delta < plength; ++delta){
	    ...
	    if(alpha == gamma){ tempel /= sqrt(2.0); }
	    if(beta == delta){ tempel /= sqrt(2.0); }
	...
	for(int r = p; r < nlength; ++r){
	  for(int s = r; s < nlength; ++s){
	    ...
	    if(num1 > num2){ std::swap(num1, num2); }
	    tempel = NN_V[nmatlength * num1 + num2];
	    if(p == q){ tempel /= sqrt(2.0); }
	    if(r == s){ tempel /= sqrt(2.0); }
	    if(fabs(tempel) > 1.0e-10){ 
	      HF_ME.set_nnJME(p, q, r, s, J, tempel);
	    }
	  }
	}
      }
	...
	for(int r = p; r < plength; ++r){
	  for(int s = 0; s < nlength; ++s){
	    ...
	    num2 = number_diff(r, s, nlength);
	    if(num1 > num2){ std::swap(num1, num2); }
	    if(fabs(PN_V[pnmatlength * num1 + num2]) > 1.0e-10){ 
