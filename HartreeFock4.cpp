		  ...
		  for(int n = 0; n < P; ++n)
		    {
		      for(int k = 0; k < P; ++k)
			{
			  //if(Space.pShells.j[n] != Space.pShells.j[k] || Space.pShells.j[m] != Space.pShells.j[l]){ continue; }
			  term = pdensmat[P * k + n] * (2*J + 1) * ME.get_ppJME(m,n,l,k,J) / (2*Space.pShells.j[m] + 1);
			  //term = pdensmat[P * k + n] * (2*J + 1) * ME.get_ppJME(m,n,l,k,J);
			  pfock[P * m + l] += term;
			}
		    }
		  for(int n = 0; n < N; ++n)
		    {
		      for(int k = 0; k < N; ++k)
			{
			  //if(Space.nShells.j[n] != Space.nShells.j[k] || Space.pShells.j[m] != Space.pShells.j[l]){ continue; }
			  term = ndensmat[N * k + n] * (2*J + 1) * ME.get_pnJME(m,n,l,k,J) / (2*Space.pShells.j[m] + 1);
			  //term = ndensmat[N * k + n] * (2*J + 1) * ME.get_pnJME(m,n,l,k,J);
			  pfock[P * m + l] += term;
			}
		    }
		}
		  ...
		  for(int n = 0; n < N; ++n)
		    {
		      for(int k = 0; k < N; ++k)
			{
			  //if(Space.nShells.j[n] != Space.nShells.j[k] || Space.nShells.j[m] != Space.nShells.j[l]){ continue; }
			  term = ndensmat[N * k + n] * (2*J + 1) * ME.get_nnJME(m,n,l,k,J) / (2*Space.nShells.j[m] + 1);
			  //term = ndensmat[N * k + n] * (2*J + 1) * ME.get_nnJME(m,n,l,k,J);
			  nfock[N * m + l] += term;
			}
		    }
		  for(int n = 0; n < P; ++n)
		    {
		      for(int k = 0; k < P; ++k)
			{
			  //if(Space.pShells.j[n] != Space.pShells.j[k] || Space.nShells.j[m] != Space.nShells.j[l]){ continue; }
			  term = pdensmat[P * k + n] * (2*J + 1) * ME.get_pnJME(n,m,k,l,J) / (2*Space.nShells.j[m] + 1);
			  //term = pdensmat[P * k + n] * (2*J + 1) * ME.get_pnJME(n,m,k,l,J);
...
{ 
  ...
  for(int J = 0; J <= Space.max2J; ++J)
	...
	{
	  for(int j = i; j < Space.Pocc; ++j)
	    {
	      hf_energy -= (2 * J + 1) * HF_ME.get_ppJME(i, j, i, j, J) / (2*Space.pShells.j[i] + 1);
	    }
	}
      std::cout << hf_energy << endl;
      for(int i = 0; i < Space.Nocc; ++i)
	{
	  for(int j = i; j < Space.Nocc; ++j)
	    {
	      hf_energy -= (2 * J + 1) * HF_ME.get_nnJME(i, j, i, j, J) / (2*Space.nShells.j[i] + 1);
	    }
	}
      std::cout << hf_energy << endl;
      for(int i = 0; i < Space.Pocc; ++i)
	{
	  for(int j = 0; j < Space.Nocc; ++j)
	    {
	      hf_energy -= (2 * J + 1) * HF_ME.get_pnJME(i, j, i, j, J) / (2*Space.pShells.j[i] + 1);
	    }
	}
    }
