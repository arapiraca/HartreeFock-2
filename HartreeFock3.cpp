...
void Separate_Particles_Holes(Single_Particle_States &States, const int &p, const int &n, const Model_Space &Space)
{
  ...
  for(int i = 0; i < Space.pshelltot; ++i)
      ...
      if(i < p)
	{
	  for(int j = 0; j < NN; ++j)
	    {
	      if(j < Space.pshelltot){ States.holes[hcount][j] = States.protons[i][j]; }
...
Single_Particle_States Build_Single_Particle_States(const Input_Parameters &Parameters, const Model_Space &Space)
{
  ...
  //double norm; // keep track of vector magnitude
  //double entry; // coefficient for each vector component
  ...
  for(int i = 0; i < Space.pleveltot; ++i)
	...
	{
	  entry = std::rand() / pow(Space.pStates.energy[j], 10);
	  States.protons[i][j] = entry; norm += pow(entry, 2.0);
	}
      for(int j = 0; j < Space.pleveltot; ++j)
	{
	  if(abs(States.protons[i][j]/sqrt(norm)) < 0.001/Space.pleveltot){ States.protons[i][j] = 0; }
	}
    }
  ...
  for(int i = 0; i < Space.nleveltot; ++i)
	...
	{
	  entry = std::rand() / pow(Space.nStates.energy[j], 10);
	  States.neutrons[i][j] = entry; norm += pow(entry, 2.0);
	}
      for(int j = 0; j < Space.nleveltot; ++j)
	{
	  if(abs(States.neutrons[i][j]/sqrt(norm)) < 0.001/Space.nleveltot){ States.neutrons[i][j] = 0; }
	}
	}*/
  ...
  for(int i = 0; i < Space.pshelltot; ++i)
    {
      for(int j = 0; j < Space.pshelltot; ++j)
	{
	  States.p_energies[i] += pow(States.protons[i][j], 2.0) * Space.pShells.energy[j];	  
	}
    }
  for(int i = 0; i < Space.nshelltot; ++i)
    {
      for(int j = 0; j < Space.nshelltot; ++j)
	{
	  States.n_energies[i] += pow(States.neutrons[i][j], 2.0) * Space.nShells.energy[j];	  
	}
    }
	      ...
	      for(int k = l + 1; k < Space.pleveltot; ++k)
		{
		  ...
		  for(int J = minJ; J <= maxJ; ++J)
		    { 
		      ...
		      factor1 = 1.0;
		      if(Space.pStates.n[m] == Space.pStates.n[n] &&
			 Space.pStates.l[m] == Space.pStates.l[n] &&
			 Space.pStates.j[m] == Space.pStates.j[n])
			{ factor1 *= 2.0 / sqrt(1.0 + pow(-1.0, J)); }
		      if(Space.pStates.n[l] == Space.pStates.n[k] &&
			 Space.pStates.l[l] == Space.pStates.l[k] &&
			 Space.pStates.j[l] == Space.pStates.j[k])
			{ factor1 *= 2.0 / sqrt(1.0 + pow(-1.0, J)); }
	      ...
	      for(int k = l + 1; k < Space.nleveltot; ++k)
		{
		  ...
		  for(int J = minJ; J <= maxJ; ++J)
		    { 
		      ...
		      factor1 = 1.0;
		      if(Space.nStates.n[m] == Space.nStates.n[n] &&
			 Space.nStates.l[m] == Space.nStates.l[n] &&
			 Space.nStates.j[m] == Space.nStates.j[n])
			{ factor1 *= 2.0 / sqrt(1.0 + pow(-1.0, J)); }
		      if(Space.nStates.n[l] == Space.nStates.n[k] &&
			 Space.nStates.l[l] == Space.nStates.l[k] &&
			 Space.nStates.j[l] == Space.nStates.j[k])
			{ factor1 *= 2.0 / sqrt(1.0 + pow(-1.0, J)); }
		      tempME += factor1 * CGC1 * CGC2 * JME;
		    }
  ...
  for(int i = 0; i < NumElements; ++i)
    {
      ...
      coupJ *= 0.5;
      //TBME = TBME - (Space.HOEnergy / Space.A) * hom;
...
Single_Particle_States Hartree_Fock_States2(const Input_Parameters &Parameters, const Single_Particle_States &States, const Model_Space &Space, const M_Matrix_Elements &ME)
{
  ...
  ind = 0;
  error = 1000;
  while((error > 0.0001 && ind < 100) || ind < 10)
    {
      std::cout << "!! " << ind << " !!" << endl;
      ...
      for(int i = 0; i < P; ++i)
	{
	  for(int j = i; j < P; ++j)
	    {
	      dens = 0.0;
	      for(int beta = 0; beta < Space.Pocc; ++beta){ dens += HF.protons[beta][i]*HF.protons[beta][j]; }
	      pdensmat[P * i + j] = dens;
      ...
      for(int i = 0; i < N; ++i)
	{
	  for(int j = i; j < N; ++j)
	    {
	      dens = 0.0;
	      for(int beta = 0; beta < Space.Nocc; ++beta){ dens += HF.neutrons[beta][i]*HF.neutrons[beta][j]; }
	      ndensmat[N * i + j] = dens;
      ...
      for(int m = 0; m < P; ++m)
	{
	  ...
	  for(int l = m; l < P; ++l)
	      ...
	      for(int n = 0; n < P; ++n)
		{
		  for(int k = 0; k < P; ++k)
		    {
		      for(int m1 = 0; m1 < int(Space.pShells.pn_levels[m].size()); ++m1)
			{
			  for(int m2 = 0; m2 < int(Space.pShells.pn_levels[n].size()); ++m2)
			    {
			      if(m == n && m1 == m2){ continue; }
			      for(int m3 = 0; m3 < int(Space.pShells.pn_levels[l].size()); ++m3)
				{
				  for(int m4 = 0; m4 < int(Space.pShells.pn_levels[k].size()); ++m4)
				    {
				      if(k == l && m3 == m4){ continue; }
				      mm = Space.pShells.pn_levels[m][m1];
				      nn = Space.pShells.pn_levels[n][m2];
				      ll = Space.pShells.pn_levels[l][m3];
				      kk = Space.pShells.pn_levels[k][m4];
				      if(Space.pStates.m[mm]+Space.pStates.m[nn] != Space.pStates.m[ll]+Space.pStates.m[kk]){ continue; }
				      pfock[P * m + l] += pdensmat[P * k + n] * ME.get_ppMME(mm,nn,ll,kk);
				      if(m != l){ pfock[P * l + m] += pdensmat[P * k + n] * ME.get_ppMME(mm,nn,ll,kk); }
				    }
				}
			    }
			}
		  ...
		  for(int k = 0; k < N; ++k)
		    {
		      for(int m1 = 0; m1 < int(Space.pShells.pn_levels[m].size()); ++m1)
			{
			  for(int m2 = 0; m2 < int(Space.nShells.pn_levels[n].size()); ++m2)
			    {
			      for(int m3 = 0; m3 < int(Space.pShells.pn_levels[l].size()); ++m3)
				{
				  for(int m4 = 0; m4 < int(Space.nShells.pn_levels[k].size()); ++m4)
				    {
				      mm = Space.pShells.pn_levels[m][m1];
				      nn = Space.nShells.pn_levels[n][m2];
				      ll = Space.pShells.pn_levels[l][m3];
				      kk = Space.nShells.pn_levels[k][m4];
				      if(Space.pStates.m[mm]+Space.nStates.m[nn] != Space.pStates.m[ll]+Space.nStates.m[kk]){ continue; }
				      pfock[P * m + l] += ndensmat[N * k + n] * ME.get_pnMME(mm,nn,ll,kk);
				      if(m != l){ pfock[P * l + m] += pdensmat[N * k + n] * ME.get_pnMME(mm,nn,ll,kk); }
				    }
				}
			    }
			}
      ...
      for(int m = 0; m < N; ++m)
	{
	  ...
	  for(int l = m; l < N; ++l)
	      ...
	      for(int n = 0; n < N; ++n)
		{
		  for(int k = 0; k < N; ++k)
		    {
		      for(int m1 = 0; m1 < int(Space.nShells.pn_levels[m].size()); ++m1)
			{
			  for(int m2 = 0; m2 < int(Space.nShells.pn_levels[n].size()); ++m2)
			    {
			      if(m == n && m1 == m2){ continue; }
			      for(int m3 = 0; m3 < int(Space.nShells.pn_levels[l].size()); ++m3)
				{
				  for(int m4 = 0; m4 < int(Space.nShells.pn_levels[k].size()); ++m4)
				    {
				      if(k == l && m3 == m4){ continue;} 
				      mm = Space.nShells.pn_levels[m][m1];
				      nn = Space.nShells.pn_levels[n][m2];
				      ll = Space.nShells.pn_levels[l][m3];
				      kk = Space.nShells.pn_levels[k][m4];
				      if(Space.nStates.m[mm]+Space.nStates.m[nn] != Space.nStates.m[ll]+Space.nStates.m[kk]){ continue; }
				      nfock[N * m + l] += ndensmat[N * k + n] * ME.get_nnMME(mm,nn,ll,kk);
				      if(m != l){ nfock[N * l + m] += ndensmat[N * k + n] * ME.get_nnMME(mm,nn,ll,kk); }
				    }
				}
			    }
			}
				  ...
				  for(int m4 = 0; m4 < int(Space.pShells.pn_levels[k].size()); ++m4)
				    {
				      ...
				      ll = Space.nShells.pn_levels[l][m3];
				      kk = Space.pShells.pn_levels[k][m4];
				      if(Space.nStates.m[mm]+Space.pStates.m[nn] != Space.nStates.m[ll]+Space.pStates.m[kk]){ continue; }
				      nfock[N * m + l] += pdensmat[P * k + n] * ME.get_pnMME(nn,mm,kk,ll);
				      if(m != l){ nfock[N * l + m] += pdensmat[P * k + n] * ME.get_pnMME(nn,mm,kk,ll); }
				    }
				}
			    }
			}
	...
	{ 
	  ...
	  for(int j = 0; j < P; ++j)
	    {
	      if(Space.pShells.j[j] != Space.pShells.j[tempind] || pow(-1.0,Space.pShells.l[j]) != pow(-1.0,Space.pShells.l[tempind]))
		{ HF.protons[i][j] = 0; }
	...
	{ 
	  ...
	  for(int j = 0; j < N; ++j)
	    {
	      if(Space.nShells.j[j] != Space.nShells.j[tempind] || pow(-1.0,Space.nShells.l[j]) != pow(-1.0,Space.nShells.l[tempind]))
		{ HF.neutrons[i][j] = 0; }
  ...
  for(int i = 0; i < Space.pshelltot - 1; ++i)
    {
      ind2 = i;
      tempen2 = HF.p_energies[i];
      for(int j = i + 1; j < Space.pshelltot; ++j)
	{
	  if(HF.p_energies[j] < tempen2){ tempen2 = HF.p_energies[j]; ind2 = j; }
	}
      std::swap(HF.p_energies[i], HF.p_energies[ind2]);
  ...
  for(int i = 0; i < Space.nshelltot - 1; ++i)
    {
      ind2 = i;
      tempen2 = HF.n_energies[i];
      for(int j = i + 1; j < Space.nshelltot; ++j)
	{
	  if(HF.n_energies[j] < tempen2){ tempen2 = HF.n_energies[j]; ind2 = j; }
	}
      std::swap(HF.n_energies[i], HF.n_energies[ind2]);
...
J_Matrix_Elements Convert_To_HF_Matrix_Elements(const string &MatrixElements, const Single_Particle_States &States, const Model_Space &Space, const J_Matrix_Elements &ME)
{
  ...
  for(int i = 0; i < plength; ++i){ HF_ME.set_nJME(i, States.n_energies[i]); }
...
{ 
  ...
  M_Matrix_Elements MME = Get_Matrix_Elements(Parameters, Space);
  Single_Particle_States States = Build_Single_Particle_States(Parameters, Space);
  Single_Particle_States HFStates = Hartree_Fock_States2(Parameters, States, Space, MME);
